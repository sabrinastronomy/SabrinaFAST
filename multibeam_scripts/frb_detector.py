...
class Classifier(object):
    def __init__(self, gdm):
        ...
        self.valid_masks = [1, 2, 3, 4, 5, 6, 7, 8, 9, 12, 13, 15, 16, 17, 24, 25, 29, 32, 33, 48, 49, 57, 64, 65, 66,
                            67, 71, 96, 97, 99, 113, 128, 130, 192, 194, 195, 256, 258, 260, 262, 263, 386, 512, 516,
                            520, 524, 525, 772, 1024, 1032, 1040, 1048, 1049, 1544, 2048, 2064, 2080, 2096, 2097, 3088,
                            4096, 4128, 4160, 4192, 4193, 4288, 6176]
...
class TextOutput(object):
    ...
    def print_html(self, data):
        if len(data['valid']) > 0:
            ...
            sys.stdout.write(
                "<tr><th align=left>SNR</th><th align=left>Time</th><th align=left>DM</th><th align=left>Filter [ms]</th><th align=left>Beam</th></tr>\n")
...
if __name__ == "__main__":
    ...
    is_valid = (is_hidden == False) & (is_noise == False) & (is_coinc_dumb == False) & (is_coinc_smart == False) & (
    is_galactic == False)
    ...
    categories["coinc_smart"] = all_cands[
        (is_hidden == False) & (is_noise == False) & (is_coinc_dumb == False) & is_coinc_smart]
    categories["galactic"] = all_cands[
        (is_hidden == False) & (is_noise == False) & (is_coinc_dumb == False) & (is_coinc_smart == False) & is_galactic]
    ...
    if len(categories['valid']) > 0 & False:
        ...
        for (i, item) in reversed(list(enumerate(categories['valid']))):
            ...
            is_not_adjacent = (is_noise == False) & (is_coinc_dumb == False) & (
            is_coinc_smart == False) & classifier.is_not_adjacent(all_cands, epoch, half_time)
            is_valid = (is_noise == False) & (is_coinc_dumb == False) & (is_coinc_smart == False) & (
            is_not_adjacent == False)
