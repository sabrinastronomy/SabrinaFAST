...
def candplots(fil_file, source_name, snr_cut, filter_cut, maxCandSec, noplot, minMem, kill_chans, kill_time_range):
    ...
    os.system(
        "frb_detector_bl.py -cands_file *_all.cand -filter_cut %d -snr_cut %f -max_cands_per_sec %f -min_members_cut %f -verbose" % (
        filter_cut, snr_cut, maxCandSec, minMem))
    os.system(
        "frb_detector_bl.py -cands_file *_all.cand -filter_cut %d -snr_cut %f -max_cands_per_sec %f -min_members_cut %f  > FRBcand" % (
        filter_cut, snr_cut, maxCandSec, minMem))
        ...
        if (frb_cands.size > 1):
            ...
            for indx, frb in enumerate(frb_cands):
                ...
                if (any(l <= time <= u for (l, u) in kill_time_range)):
                    print
                    "Candidate inside bad-time range"
                else:
                    os.system("dspsrfil -S %f -c %f -T %f -t 12 -D %f  -O %04d_%fsec_DM%f -e ar %s" % (
                    stime, extime, extime, dm, indx, time, dm, fil_file))
        elif (frb_cands.size):
            ...
            if (any(l <= time <= u for (l, u) in kill_time_range)):
                ...
                os.system("dspsrfil -cepoch=start -S %f -c %f -T %f -t 12 -D %f  -O 0000_%fsec_DM%f -e ar %s" % (
                stime, extime, extime, dm, time, dm, fil_file))
        else:
            print
            "No candidate found"
            return
...
def heimdall_run(fil_file, dmlo, dmhi, base_name, boxcar_max, dorfi, kill_chan_range):
    print
    "Running Heimdal with %f to %f DM range" % (lodm, hidm)
    # Test
    ...
    if dorfi is True:
        ...
        cmd = "heimdall -f %s -scrunching 1 -scrunching_tol 1.05 -rfi_tol 3 -dm_nbits 32 -dm_pulse_width 1000 -dm_tol 1.05 -dm %f %f -boxcar_max %f -output_dir %s/  -v %s" % (
        fil_file, dmlo, dmhi, boxcar_max, base_name, zapchan)
        # print cmd
        os.system(cmd)
    else:
        os.system("heimdall -f %s -dm_tol 1.01 -dm %f %f -boxcar_max %f -output_dir %s/  -v" % (
        fil_file, dmlo, dmhi, boxcar_max, base_name));
    return
...
if __name__ == "__main__":
    ...
    print
    "Output will go to %s/%s" % (outdir, base_name)
    ...
    os.chdir(basedir)
    if (dorfi is True):
        kill_chans, kill_chan_range, kill_time_range = rfi(fil_file, time, timesig, freqsig, chanfrac, intfrac,
                                                           max_percent, mask, sp)
    else:
        kill_chans = []
        kill_chan_range = []
