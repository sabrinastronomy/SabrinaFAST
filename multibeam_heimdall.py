...
def run_heimdall(input_file, output_directory, dm_min, dm_max, dm_tol=1.01, rfi_tolerance=3, boxcar_max=16):
    subprocess.call("heimdall -f " + input_file + " -output_dir " + output_directory + " -dm " + str(dm_min) + " " + str(dm_max) + " -dm_tol " + "1.01" + " -rfi_tol " + str(rfi_tolerance) + " -boxcar_max " + str(boxcar_max), shell = True)
